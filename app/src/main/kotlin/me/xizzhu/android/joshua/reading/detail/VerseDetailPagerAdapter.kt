...
import me.xizzhu.android.joshua.databinding.PageVerseDetailAiBinding
...
class VerseDetailPagerAdapter(context: Context) : RecyclerView.Adapter<VerseDetailPage<*>>() {
    ...
    private var requestAiExplanation: (() -> Unit)? = null
    ...
    fun initialize(updateNote: (VerseIndex, String) -> Unit, requestCrossReferences: () -> Unit, requestStrongNumber: () -> Unit, requestAiExplanation: () -> Unit) {
        this.updateNote = updateNote
        this.requestCrossReferences = requestCrossReferences
        this.requestStrongNumber = requestStrongNumber
        this.requestAiExplanation = requestAiExplanation
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VerseDetailPage<*> =
            when (viewType) {
                ...
                PAGE_AI -> AiPage(inflater, parent, requestAiExplanation!!)
    ...
    fun pageTitle(position: Int): CharSequence = when (position) {
        ...
        PAGE_AI -> resources.getString(R.string.text_ai_explanation)
...
private class AiPage(inflater: LayoutInflater, container: ViewGroup, private val requestAiExplanation: () -> Unit)
    : VerseDetailPage<PageVerseDetailAiBinding>(PageVerseDetailAiBinding.inflate(inflater, container, false)) {

    init {
        // Tapping the prompt text (or an error message) triggers a fresh explanation fetch
        viewBinding.aiPrompt.setOnClickListener { requestAiExplanation() }
    }
    ...
    private fun showPrompt() {
        viewBinding.aiLoading.visibility = android.view.View.GONE
        viewBinding.aiScroll.visibility = android.view.View.GONE
        viewBinding.aiPrompt.visibility = android.view.View.VISIBLE
        // Reset prompt text in case it was previously showing an error
        viewBinding.aiPrompt.setText(me.xizzhu.android.joshua.R.string.text_ai_tap_to_explain)
    }

    private fun showLoading() {
        viewBinding.aiPrompt.visibility = android.view.View.GONE
        viewBinding.aiScroll.visibility = android.view.View.GONE
        viewBinding.aiLoading.visibility = android.view.View.VISIBLE
    }

    private fun showExplanation(text: String, settings: Settings) {
        viewBinding.aiLoading.visibility = android.view.View.GONE
        viewBinding.aiPrompt.visibility = android.view.View.GONE
        viewBinding.aiScroll.visibility = android.view.View.VISIBLE
        viewBinding.aiExplanation.text = text
        viewBinding.aiExplanation.setPrimaryTextSize(settings)
    }

    /** Call this when the API returns an error so the user can retry. */
    fun showError(message: String) {
        viewBinding.aiLoading.visibility = android.view.View.GONE
        viewBinding.aiScroll.visibility = android.view.View.GONE
        viewBinding.aiPrompt.visibility = android.view.View.VISIBLE
        viewBinding.aiPrompt.text = message
    }
}
