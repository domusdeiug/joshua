...
class VerseDetailViewLayout : FrameLayout {
    companion object {
        ...
        const val VERSE_DETAIL_AI = 4

        @IntDef(VERSE_DETAIL_VERSES, VERSE_DETAIL_NOTE, VERSE_DETAIL_CROSS_REFERENCES, VERSE_DETAIL_STRONG_NUMBER, VERSE_DETAIL_AI)
    ...
    fun show(@VerseDetail verseDetail: Int) {
        ...
        viewBinding.viewPager.currentItem = when (verseDetail) {
            ...
            VERSE_DETAIL_AI -> VerseDetailPagerAdapter.PAGE_AI
            else -> 0
        }
    }

    fun setAiExplanation(explanation: String) {
        adapter.verseDetail?.aiExplanation = explanation
        adapter.notifyItemChanged(VerseDetailPagerAdapter.PAGE_AI)
    }

    fun setAiLoading() {
        adapter.verseDetail?.aiExplanation = ""  // empty = loading sentinel
        adapter.notifyItemChanged(VerseDetailPagerAdapter.PAGE_AI)
    }

    fun setAiError(message: String) {
        adapter.verseDetail?.aiExplanation = null  // reset so prompt shows with error text
        adapter.notifyItemChanged(VerseDetailPagerAdapter.PAGE_AI)
    }
